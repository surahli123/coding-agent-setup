...
1. **Parallel-work check** — `gh pr list --state all --limit 10` and `git log --all --oneline -15`. Surface in-flight PRs from Codex/other agents/yourself. Do not proceed until the user confirms no overlap. (Why — a PR-duplication incident: `~/.claude/refs/rule-rationale.md`.)
