...
**Why this exists:** Cross-review is the user's #1 friction surface (per /insights). The wins are real (caught a P1 NameError; caught a commit misattribution where the wrong SHA was blamed, via disk-arbitration). The failures are repeatable: diffing local WIP instead of the pristine ref; reviewing source that a parallel Codex shell changed mid-review; committing onto another agent's branch. This protocol gates each failure mode.
...
Rationale (per insights): sessions WITH Codex adversarial review had ~3x lower wrong-approach friction. Independent context catches blind spots same-context review misses.
...
- Detect the source-of-truth ref (the team lead's pristine ref / real PR base — NOT your local modified branch).
...
- **Evidence:** a review was once invalidated because Claude diffed local WIP instead of the team lead's pristine ref.
...
- **Evidence:** a parallel Codex shell had already patched a P0 bug earlier reviewers flagged; re-grepping HEAD refuted the stale claims.
...
- **Evidence:** undetected parallel Codex push → a PR conflict surfaced 20 min later.
...
- **Evidence:** Codex claimed a change belonged to one commit; disk-arbitration found the real commit.
